...
Primary transport connector for the IMOS Communications Engine,
handling Twilio webhooks and PSTN calls.
"""
...
class TwilioConnector(TransportConnector):
    """Twilio transport connector for PSTN calls"""

    @property
    def transport_type(self) -> TransportType:
        return TransportType.TWILIO

    async def validate_request(self, request_data: Dict[str, Any]) -> bool:
        """Validate Twilio webhook request"""
        try:
            # Twilio sends specific headers and data format
            ...
            # Validate call SID format (Twilio SIDs start with 'CA')
    ...
    async def extract_call_data(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract call data from Twilio webhook format"""
        try:
            # Twilio webhook data structure
            call_data = {
                'session_id': request_data.get('CallSid'),
                'phone_number': request_data.get('From'),
                'direction': request_data.get('Direction', 'inbound'),
                'status': self._map_twilio_status(request_data.get('CallStatus', 'unknown')),
            ...
            # Add additional Twilio-specific data
            call_data['transport_data'] = {
                'twilio_sid': request_data.get('CallSid'),
                'to_number': request_data.get('To'),
                'from_country': request_data.get('FromCountry'),
                'to_country': request_data.get('ToCountry'),
                'recording_url': request_data.get('RecordingUrl'),
                'digits': request_data.get('Digits'),  # DTMF input
                'speech_result': request_data.get('SpeechResult'),  # Speech recognition
                'confidence': request_data.get('Confidence'),  # Speech confidence
            ...
            logger.error(f"Error extracting Twilio call data: {e}")
            raise

    async def send_response(self, session: CallSession, response_data: Dict[str, Any]) -> bool:
        """Send response back through Twilio"""
        try:
            # For Twilio, responses are sent as TwiML XML
            response_type = response_data.get('type', 'text')

            if response_type == 'text':
                # Send text response (will be converted to speech)
                text = response_data.get('text', '')
                logger.info(f"Sending text response to Twilio: {text[:50]}...")

            elif response_type == 'audio':
                # Send audio URL
                audio_url = response_data.get('audio_url', '')
                logger.info(f"Sending audio response to Twilio: {audio_url}")

            elif response_type == 'hangup':
                # End the call
                logger.info("Sending hangup command to Twilio")
                return await self.end_call(session)

            # In a real implementation, this would return TwiML XML
            # For now, we just log the action
            return True

        except Exception as e:
            logger.error(f"Error sending response via Twilio: {e}")
            return False

    async def end_call(self, session: CallSession) -> bool:
        """End the call through Twilio"""
        try:
            # In a real implementation, this would call Twilio's API to hang up
            logger.info(f"Ending Twilio call for session {session.session_id}")
            return True
    ...
    def _detect_language(self, request_data: Dict[str, Any]) -> str:
        """Detect language from Twilio data"""
    ...
    def _detect_dialect(self, request_data: Dict[str, Any]) -> str:
        """Detect dialect from Twilio data"""
