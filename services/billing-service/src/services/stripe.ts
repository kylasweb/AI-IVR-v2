...
export class StripeService {
    ...
    async createOrRetrieveCustomer(email: string, name?: string, metadata?: Record<string, string>) {
        try {
            ...
            logger.error('Error creating/retrieving Stripe customer', { error, email });
            throw error;
        }
    }
    ...
    async createSubscription(customerId: string, priceId: string, metadata?: Record<string, string>) {
        try {
            ...
            logger.error('Error creating Stripe subscription', { error, customerId, priceId });
            throw error;
        }
    }
    ...
    async listCustomerSubscriptions(customerId: string) {
        try {
            ...
            logger.error('Error listing customer subscriptions', { error, customerId });
            throw error;
        }
    }
